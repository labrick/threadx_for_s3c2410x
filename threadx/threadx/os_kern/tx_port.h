...
#define TX_INT_DISABLE              0x80   /* Disable interrupts value */
...
/* Define ThreadX interrupt lockout and restore macros for protection on 
   ...
   macros.  */
//__swi(0x00) int  swi_handle(int operation,  int parment);
extern int swi_handle(int x, int y);

#define TX_INTERRUPT_SAVE_AREA      unsigned int cpsr_save;
#define TX_DISABLE                  cpsr_save   = swi_handle(0,0);
#define TX_RESTORE                  swi_handle(1, cpsr_save);
#define _tx_thread_schedule()       swi_handle(2, 0)
#define _tx_thread_system_return()  swi_handle(3, 0)
//__swi(0x01) int  read_cpsr(void);
//__swi(0x01) void  write_cpsr(int cpsr_save);
//__swi(0x02) void  _tx_thread_system_return(void);
//__swi(0x03) void  _tx_thread_schedule(void);
